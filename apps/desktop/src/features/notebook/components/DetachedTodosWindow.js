...
export const DetachedTodosWindow = () => {
    ...
    return (_jsx("main", { className: "app-shell detached-todos-window", "data-theme": theme, children: _jsx("div", { className: "detached-todos-window-content", children: _jsx(NotebookTodosPanel, { todos: todos, runningTodoIds: runningTodoIds, onAddTodo: (description, options) => void sendTodosCommand({ type: "add", description, ...options }), onSaveTodo: (todo) => void sendTodosCommand({ type: "save", todo }), onDeleteTodo: (todoId) => void sendTodosCommand({ type: "delete", todoId }), onAddNote: (todoId) => void sendTodosCommand({ type: "add-note", todoId }), onToggleTime: (todoId, isRunning) => void sendTodosCommand({ type: "toggle-time", todoId, isRunning }), headerActions: (_jsx("div", { className: "notebook-todos-window-actions", children: _jsx("span", { className: "tiny-text", children: isConnected ? "Synced with NoteSmith" : "Connecting..." }) })) }) }) }));
};
