...
export const SessionsSidebar = ({ sessions, activeSessionId, onSelect, onCreate, onDelete, }) => {
    ...
    const filteredSessions = useMemo(() => {
        ...
        if (!query)
            return sessions;
        return sessions.filter((session) => [session.title, session.date, session.participantText, session.project, session.department, session.tagsText, session.manualNotes, session.liveTranscript]
            .join(" ")
            .toLowerCase()
            .includes(query));
    }, [filter, sessions]);
