...
describe("SessionEditor template options", () => {
    it("adds meeting templates to Notebook-opened quick-note sessions", () => {
        ...
        const notebookOptions = getSessionEditorTemplateOptions(BUILTIN_TEMPLATES, "quick-note", ["quick-note", "meeting-note"]);
        expect(ordinaryQuickNoteOptions.map((template) => template.id)).not.toContain("meeting");
        expect(notebookOptions.map((template) => template.id)).toEqual(expect.arrayContaining(["personal-note", "one-on-one", "meeting"]));
    });
});
