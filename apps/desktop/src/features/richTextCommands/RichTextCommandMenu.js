...
const formatLocalDate = (value) => `${value.getFullYear()}-${pad(value.getMonth() + 1)}-${pad(value.getDate())}`;
const formatLocalTime = (value) => `${pad(value.getHours())}:${pad(value.getMinutes())}`;
...
export const resolveRichTextCommandValue = (template, now = new Date()) => {
    ...
    return template.replace(/\{(date|time|datetime|tomorrow|yesterday|week|day)\}/g, (_, key) => replacements[key]);
};
export const buildRichTextCommands = (customCommands = []) => {
...
export const findRichTextCommandQuery = (text, offset = text.length) => {
...
export const RichTextCommandMenu = ({ editorRef, onContentChange }) => {
    ...
    useEffect(() => {
        ...
        const insertCommand = (command, suffix) => {
            const match = getCommandQuery(editor);
            if (!match)
                return;
    ...
    return (_jsxs("div", { className: "rich-text-command-menu", role: "listbox", "aria-label": "Text commands", style: { top: activeQuery.top, left: activeQuery.left }, children: [_jsx("div", { className: "rich-text-command-menu-heading", children: "Text commands" }), filteredCommands.map((command, index) => (_jsxs("button", { type: "button", role: "option", "aria-selected": index === activeIndex, "data-active": index === activeIndex, onMouseDown: (event) => event.preventDefault(), onClick: () => {
                ...
                }, children: [_jsxs("strong", { children: ["@", command.trigger] }), _jsx("span", { children: command.label }), _jsx("small", { children: command.description })] }, command.trigger))), _jsx("div", { className: "rich-text-command-menu-hint", children: "Enter or Tab to insert | Esc to close" })] }));
};
