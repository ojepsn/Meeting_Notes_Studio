...
export const CalendarWorkspace = ({ todos, activities, timeLogs, calendarItems, settings, structureOptions, linkedSessionStateByActivity, onSaveSettings, onCreateFromText, onMoveItem, onSaveTodo, onDeleteTodo, onSaveActivity, onDeleteActivity, onConvertTodoToActivity, onConvertTodoToMeeting, onRemoveCalendarItems, onUpdateCalendarItem, onStartTracking, onStopTracking, onOpenTodoWorkspace, onOpenTodoDetail, onOpenActivityWorkspace, onOpenActivityDetail, onOpenSession, highlightedItemId, onCreateLinkedMeetingSession, onPreviewSessionOutput, onFullScreenChange, }) => {
    ...
    const items = useMemo(() => {
        ...
        const base = (Array.isArray(calendarItems) ? calendarItems : [])
            ...
            if (item.targetType === "todo") {
                ...
                if (!todo)
                    return null;
                return { id: item.id, date: item.date, startSlot: item.startSlot, durationSlots: item.durationSlots, targetType: "todo", targetId: item.targetId, title: todo.description, label: "Todo", isMeeting: false, isPrivate: todo.isPrivate, isDone: todo.isDone, lane: 0, laneCount: 1 };
            }
            const activity = activityMap.get(item.targetId);
            if (!activity)
                return null;
            return { id: item.id, date: item.date, startSlot: item.startSlot, durationSlots: item.durationSlots, targetType: "activity", targetId: item.targetId, title: activity.description, label: activity.type === "meeting" ? "Meeting" : "Activity", isMeeting: activity.type === "meeting", isPrivate: activity.isPrivate, isDone: activity.isDone, lane: 0, laneCount: 1 };
        })
    ...
    const removeCompletedTodosFromGrid = () => {
        if (!completedTodoCalendarItemIds.length)
            return;
