...
export const CalendarWorkspace = ({ todos, activities, timeLogs, calendarItems, settings, structureOptions, linkedSessionStateByActivity, linkedSessionStateByTodo, onSaveSettings, onCreateFromText, onMoveItem, onSaveTodo, onDeleteTodo, onSaveActivity, onDeleteActivity, onConvertTodoToMeeting, onUpdateCalendarItem, onStartTracking, onStopTracking, onOpenTodoWorkspace, onOpenTodoDetail, onOpenActivityWorkspace, onOpenActivityDetail, onOpenSession, highlightedItemId, onCreateLinkedMeetingSession, onCreateLinkedTaskSession, onPreviewSessionOutput, onFullScreenChange, }) => {
    ...
    const [visibilityFilter, setVisibilityFilter] = useState(settings.calendarVisibilityFilter ?? "all");
    ...
    const items = useMemo(() => {
        ...
        return (Array.isArray(calendarItems) ? calendarItems : [])
            ...
            if (item.targetType === "todo") {
                ...
                if (!todo)
                    return null;
                return { id: item.id, date: item.date, startSlot: item.startSlot, durationSlots: item.durationSlots, targetType: "todo", targetId: item.targetId, title: todo.description, label: "Task", isMeeting: false, isPrivate: todo.isPrivate, isDone: todo.isDone, isPriority: Boolean(todo.isPriority), lane: 0, laneCount: 1 };
            }
                                                ...
                                                void createMeetingFromGrid(date, Math.floor((event.clientY - rect.top) / slotHeight));
                                                    ...
                                                    return _jsxs("div", { className: `calendar-item-block calendar-item-block-${item.targetType}${item.isMeeting ? " calendar-item-block-meeting" : ""}${item.targetType === "todo" && item.isPriority ? " calendar-item-block-priority-todo" : ""}${item.targetType === "todo" && item.isDone ? " calendar-item-block-completed-todo" : ""}${isPastCalendarItem ? " calendar-item-block-past" : ""}${isSelected ? " calendar-item-block-selected" : ""}${selectedItemIds.length > 1 && selectedItemIds.includes(item.id) ? " calendar-item-block-multi-selected" : ""}${inlineTodoEditForItem ? " calendar-item-block-inline-editing" : ""}${sizeClass}`, role: "button", tabIndex: 0, style: { top: `calc(var(--calendar-slot-height) * ${startSlot} + 2px)`, height: `${visualHeight}px`, width: `calc(${laneWidth}% - 8px)`, left: `calc(${item.lane * laneWidth}% + 4px)`, right: "auto" }, onMouseDown: (event) => {
                                                            ...
                                                            return;
                                                                    ...
                                                                    if (event.key === "Enter") {
                                                                        ...
                                                                        onCreateLinkedMeetingSession(item.targetId);
                                                                    }, children: "Create session" })) })) : null, !item.isMeeting && item.targetType === "todo" ? (_jsx("div", { className: "calendar-item-launcher-row", children: linkedSessionState?.sessionId ? (_jsxs(_Fragment, { children: [_jsx("span", { className: "calendar-item-inline-action calendar-item-inline-action-secondary", role: "button", tabIndex: -1, onMouseDown: (event) => {
                                                                                event.preventDefault();
                                                                                event.stopPropagation();
                                                                            }, onClick: (event) => {
                                                                                event.preventDefault();
                                                                                event.stopPropagation();
                                                                                onOpenSession(linkedSessionState.sessionId, item.id);
                                                                            }, children: "Open note" }), linkedSessionState.hasOutput ? (_jsx("span", { className: "calendar-item-inline-action calendar-item-inline-action-secondary", role: "button", tabIndex: -1, onMouseDown: (event) => {
                                                                                event.preventDefault();
                                                                                event.stopPropagation();
                                                                            }, onClick: (event) => {
                                                                                event.preventDefault();
                                                                        ...
                                                                        event.stopPropagation();
                                                                        onCreateLinkedTaskSession(item.targetId);
                                                                    }, children: "Create note" })) })) : null, _jsx("span", { className: `calendar-item-inline-action${runningLog ? " calendar-item-inline-action-active" : ""}`, role: "button", tabIndex: -1, onMouseDown: (event) => {
                                                                    event.preventDefault();
                                                                    event.stopPropagation();
                                                                }, onClick: (event) => {
                                                                    event.preventDefault();
