...
export const App = () => {
    ...
    const selectedNewSessionTemplate = quickStartTemplates.find((template) => template.id === selectedNewSessionTemplateId) ??
    ...
    const openNotesTarget = ({ sessionId, view = "capture", returnWorkspace = null, status, }) => {
        clearRequestedFilters();
        setLinkedDetailReturnWorkspace(returnWorkspace);
    ...
    const openSessionFromLink = (sessionId, returnWorkspace = null) => openNotesTarget({
        sessionId,
        view: "capture",
        returnWorkspace,
    ...
    const handleCreateSessionFromTemplate = async (templateId) => {
        const template = snapshot?.templates.find((entry) => entry.id === (templateId || selectedNewSessionTemplate?.id)) ??
    ...
    const renderOverlayContent = () => {
        switch (openPanel) {
            ...
            case "new-note":
                return (_jsxs("div", { className: "sidebar-card overlay-card", children: [_jsxs("div", { children: [_jsx("h3", { children: "New session" }), _jsx("p", { children: "Select the template for the next session, then create it." })] }), _jsxs("div", { className: "session-quick-start-row", children: [_jsx("button", { className: "primary-button session-new-button", type: "button", onClick: () => void handleCreateSessionFromTemplate(), children: "New" }), _jsx("div", { className: "session-template-pill-row", children: quickStartTemplates.map((template) => (_jsx("button", { type: "button", className: "segment-button session-template-pill", "data-active": selectedNewSessionTemplate?.id === template.id, onClick: () => setSelectedNewSessionTemplateId(template.id), children: template.name }, template.id))) })] })] }));
            case "instructions":
