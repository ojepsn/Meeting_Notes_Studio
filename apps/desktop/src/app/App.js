...
import { getRunningTodoIds, openDetachedTodosWindow, TODOS_COMMAND_EVENT, sendTodosSnapshot, } from "../features/notebook/todosWindowBridge";
...
import { fileToAttachmentRecord, loadPersistedAttachmentFile, pickAudioFile, pickImageFile, pickTranscriptFile, persistGeneratedAttachment, persistSelectedAttachment, readTranscriptFile, removePersistedAttachment, restoreImportedAttachmentFiles, } from "../lib/files/attachmentStore";
...
const SECONDARY_WORKSPACE_ITEMS = WORKSPACE_ITEMS.filter((item) => !["notebook", "calendar", "notes", "now", "todos"].includes(item.id));
const SINGLE_PANE_WORKSPACES = ["notebook", "todos", "time", "now", "analytics", "structure"];
const HIDE_SHARED_INSPECTOR_WORKSPACES = ["notebook", "notes", "todos", "time", "analytics", "now", "structure"];
...
export const App = () => {
    const { snapshot, activeSessionId, saveState, lastSavedAt, isLoaded, loadError, load, setActiveSessionId, activeView, setActiveView, repository, saveSession, createNewSession, deleteSession, restoreSession, permanentlyDeleteSession, saveTodo, addTodo, deleteTodo, saveActivity, addActivity, deleteActivity, saveChecklist, createChecklist, deleteChecklist, saveChecklistTemplate, createChecklistTemplate, deleteChecklistTemplate, createChecklistFromTemplate, createChecklistRecurrence, deleteChecklistRecurrence, saveTimeLog, deleteTimeLog, startTimeTracking, stopTimeTracking, startWorkBaseline, stopWorkBaseline, startAdhocTimeLog, createCalendarEntryFromText, rollForwardOverdueTodos, moveCalendarItem, updateCalendarItem, convertTodoToActivity, ensureSessionForActivity, ensureSessionForTodo, ensureTimeTargetForSession, saveSettings, renameDomainValue, renameProjectValue, saveTemplate, resetTemplates, importLegacyBrowserData, importBackupSnapshot: restoreBackupSnapshot, saveAttachments, } = useDesktopStore();
    const [activeWorkspace, setActiveWorkspace] = useState("notebook");
    ...
    const [notesWorkspaceTab, setNotesWorkspaceTab] = useState("capture");
    ...
    const buildDesktopTransferBackupBundle = async () => {
    ...
    const sortParticipantText = (participantText) => parsePeopleFromSession(participantText)
        ...
        .join(", ");
    const rankSavedValues = (items, savedValues, collectEntries) => {
    ...
    const suggestedPeople = useMemo(() => {
        ...
        return rankSavedValues(participantSuggestionSources, snapshot.settings.savedParticipants, (item) => parsePeopleFromSession(item.participantText ?? ""));
    ...
    const activeSessionTimeTarget = activeLinkedTodo
        ...
        : activeLinkedActivity
            ...
            : null;
    const isActiveSessionTimeTracking = Boolean(activeSessionTimeTarget && snapshot?.timelogs.some((log) => log.targetType === activeSessionTimeTarget.targetType
    ...
    const toggleActiveSessionTimeTracking = async () => {
        ...
        const isRunning = Boolean(latestSnapshot?.timelogs.some((log) => log.targetType === target.targetType && log.targetId === target.targetId && log.startTime === log.endTime));
        if (isRunning) {
    ...
    const openTimeLogs = useMemo(() => snapshot?.timelogs.filter((entry) => entry.startTime === entry.endTime) ?? [], [snapshot]);
    ...
    const isBaselineWorkRunning = openTimeLogs.some((entry) => entry.targetType === "activity" && entry.targetId === baselineWorkActivityId);
    const hasSpecificRunningTimeLog = openTimeLogs.some((entry) => !(entry.targetType === "activity" && entry.targetId === baselineWorkActivityId));
    ...
    const loadPersistedAudioFileForSession = async (sessionId) => {
        const latestSnapshot = await repository.loadSnapshot();
        const persistedAudioAttachment = latestSnapshot.attachments.find((entry) => entry.sessionId === sessionId && entry.kind === "audio") ?? null;
    ...
    const handleImportBackup = async (mode = "replace") => {
        try {
            ...
            if (imported.kind === "pwa-export") {
                ...
                const importedSnapshotWithFiles = imported.attachmentFiles?.length
                    ...
                    : imported.snapshot;
                const nextSnapshot = mode === "merge" && snapshot
    ...
    const handleCopyManualNotesDirect = async () => {
        ...
        setIsGenerating(true);
        setGenerationLog([]);
        appendGenerationLog("Direct Manual-notes copy started.", "info", `session: ${visibleSession.title || "Untitled session"}\nsource: Manual notes only`);
        try {
            setSelectedOutputVersionId(null);
            await saveSession({ ...visibleSession, ...buildOutputVersionPatch(visibleSession, directOutput) });
            appendGenerationLog("Manual notes copied directly to Output.", "success", `output characters: ${directOutput.length}`);
            ...
            appendGenerationLog("Direct Manual-notes copy failed.", "error", error instanceof Error ? error.message : String(error));
            setStatusNote(error instanceof Error ? error.message : "Manual notes could not be copied directly to Output.");
        }
        finally {
            setIsGenerating(false);
        }
    ...
    const handleCreateInlineImageAttachment = async (file) => {
    ...
    const handleRemoveAttachment = async (attachmentId) => {
        ...
        if (attachment.kind === "image" && attachment.sessionId === activeSession.id) {
            const cleanedManualNotes = activeSession.manualNotes.replace(new RegExp(`<figure[^>]*data-notesmith-attachment-id="${attachment.id}"[^>]*>.*?<\\/figure>(?:<p><br><\\/p>)?`, "gis"), "");
    ...
    const openLinkedDestination = ({ workspace, todoId = null, todoDomain = null, todoProject = null, timeDomain = null, timeProject = null, returnWorkspace = null, status, }) => {
    ...
    return (_jsx(RichTextCommandProvider, { customCommands: snapshot.settings.richTextCommands, spellCheckMode: snapshot.settings.richTextSpellCheck, children: _jsxs("div", { className: "app-shell desktop-shell", "data-theme": snapshot.settings.theme, "data-workspace-rail-collapsed": isWorkspaceRailCollapsed, onKeyDownCapture: (event) => void handleGlobalTodoShortcut(event), children: [_jsxs("aside", { className: "workspace-rail", "data-collapsed": isWorkspaceRailCollapsed, children: [_jsxs("div", { className: "workspace-rail-brand", children: [_jsxs("div", { className: "workspace-rail-brand-copy", children: [_jsx("strong", { children: "NoteSmith" }), _jsx("span", { className: "tiny-text", children: "Desktop" })] }), _jsx("button", { className: "workspace-rail-toggle", type: "button", "aria-label": isWorkspaceRailCollapsed ? "Expand workspace navigation" : "Collapse workspace navigation", "aria-expanded": !isWorkspaceRailCollapsed, title: isWorkspaceRailCollapsed ? "Expand workspace navigation" : "Collapse workspace navigation", onClick: () => setIsWorkspaceRailCollapsed((current) => !current), children: isWorkspaceRailCollapsed ? ">" : "<" })] }), _jsxs("nav", { className: "workspace-nav", children: [PRIMARY_WORKSPACE_ITEMS.map((item) => (_jsxs("button", { type: "button", className: "workspace-nav-button", "data-active": activeWorkspace === item.id, "data-available": item.available, onClick: () => handleWorkspaceSelection(item.id, item.available), children: [_jsx("span", { children: item.label }), _jsx("small", { children: item.available ? item.description : "Coming later" })] }, item.id))), DAILY_WORKSPACE_ITEMS.map((item) => (_jsxs("button", { type: "button", className: "workspace-nav-button", "data-active": activeWorkspace === item.id, "data-available": item.available, onClick: () => handleWorkspaceSelection(item.id, item.available), children: [_jsx("span", { children: item.label }), _jsx("small", { children: item.available ? item.description : "Coming later" })] }, item.id))), _jsx(TodosRailCard, { active: activeWorkspace === "todos", onOpen: () => setActiveWorkspace("todos") }), SECONDARY_WORKSPACE_ITEMS.map((item) => (_jsxs("button", { type: "button", className: "workspace-nav-button", "data-active": activeWorkspace === item.id, "data-available": item.available, onClick: () => handleWorkspaceSelection(item.id, item.available), children: [_jsx("span", { children: item.label }), _jsx("small", { children: item.available ? item.description : "Coming later" })] }, item.id)))] })] }), _jsxs("div", { className: "workspace-shell", children: [activeWorkspace !== "notebook" ? (_jsxs("button", { className: "workspace-todos-edge-tab", type: "button", "data-active": activeWorkspace === "todos", "aria-label": "Open Todos", title: "Open Todos", onClick: handleOpenWorkspaceTodos, children: [_jsx("span", { children: "<" }), _jsx("strong", { children: "Todos" })] })) : null, _jsxs("header", { className: `topbar app-header${activeWorkspace === "notes" ? " app-header-notes-pwa" : ""}${activeWorkspace === "notebook" ? " app-header-compact" : ""}${activeWorkspace === "calendar" ? " app-header-compact app-header-calendar-home" : ""}${activeWorkspace === "calendar" && isCalendarWorkspaceFullScreen ? " app-header-compact" : ""}`, children: [_jsx("div", { className: "topbar-copy", children: activeWorkspace === "notes" ? (_jsx("div", { className: "topbar-status-strip topbar-status-strip-notes" })) : (_jsxs(_Fragment, { children: [_jsx("div", { className: "topbar-eyebrow", children: "Focused workspace" }), _jsx("h1", { children: `${WORKSPACE_ITEMS.find((item) => item.id === activeWorkspace)?.label || "Workspace"}` }), _jsxs("div", { className: "topbar-status-strip", children: [_jsx("span", { className: `status-chip status-chip-${saveState}`, children: saveStatusLabel }), _jsx("span", { className: "status-chip", children: desktopVersion ? `v${desktopVersion}` : "Desktop" }), _jsx("span", { className: "status-chip", children: baselineWorkStatusLabel }), activeWorkspace !== "calendar" ? _jsx("span", { className: "status-chip", children: aiActivityLabel }) : null, activeWorkspace !== "calendar" ? (_jsxs(_Fragment, { children: [_jsx("span", { className: "status-chip", children: selectedTextModelOption?.label || snapshot.settings.textModel }), _jsx("span", { className: "status-chip", children: selectedTranscriptionModelOption?.label || snapshot.settings.transcriptionModel })] })) : null, isCheckingForUpdates ? _jsx("span", { className: "status-chip", children: "Checking updates..." }) : null] }), _jsx("span", { className: "tiny-text topbar-status-note", children: statusNote })] })) }), _jsx("div", { className: "topbar-actions topbar-actions-split", children: _jsxs("div", { className: "topbar-secondary-cluster", children: [activeWorkspace === "notes" && linkedDetailReturnWorkspace === "calendar" ? (_jsx("button", { className: "primary-button", type: "button", onClick: returnFromLinkedDetail, children: "Back to Calendar" })) : null, activeWorkspace === "notes" ? (_jsxs(_Fragment, { children: [_jsx("button", { className: "shell-button", type: "button", "aria-pressed": isNotesSessionsOpen, onClick: () => setIsNotesSessionsOpen((current) => !current), children: "Sessions" }), _jsx("button", { className: "shell-button", type: "button", onClick: () => openOverlay("backup"), children: "Back-up" }), _jsx("button", { className: "shell-button", type: "button", onClick: () => openOverlay("instructions"), children: "Instructions" }), _jsx("button", { className: "shell-button", type: "button", onClick: () => openSettingsSection("ai"), children: "Settings" }), _jsx("span", { className: `status-chip status-chip-${saveState}`, children: saveStatusLabel })] })) : null, activeWorkspace !== "notes" ? (_jsxs(_Fragment, { children: [_jsx("button", { className: "shell-button", type: "button", onClick: openCommandPalette, children: "Command palette" }), _jsx("button", { className: "shell-button", type: "button", onClick: () => void (availableUpdateVersion ? handleInstallUpdate() : handleCheckForUpdates()), children: availableUpdateVersion ? `Install ${availableUpdateVersion}` : "Check updates" }), _jsx("button", { className: isBaselineWorkEnabled ? "primary-button" : "shell-button", type: "button", onClick: () => void (isBaselineWorkEnabled ? stopWorkBaseline() : startWorkBaseline()), children: isBaselineWorkEnabled ? "Stop work" : "Start work" }), _jsx("button", { className: "shell-button", type: "button", onClick: () => openSettingsSection("ai"), children: "Settings" }), _jsx("button", { className: "shell-button", type: "button", onClick: () => openOverlay("sessions"), children: "All Sessions" }), _jsx("button", { className: "shell-button", type: "button", onClick: () => openOverlay("more"), children: "More" })] })) : null] }) })] }), availableUpdateVersion ? (_jsxs("div", { className: "workspace-alert-bar", children: [_jsxs("span", { children: ["Desktop update ", availableUpdateVersion, " is available from GitHub Releases."] }), _jsx("button", { className: "primary-button", type: "button", onClick: () => void handleInstallUpdate(), disabled: isInstallingUpdate, children: isInstallingUpdate ? "Installing update..." : `Install update ${availableUpdateVersion}` }), manualUpdateUrl ? (_jsx("button", { className: "small-button", type: "button", onClick: () => void handleOpenManualUpdate(), disabled: isInstallingUpdate, children: "Download installer" })) : null] })) : manualUpdateUrl ? (_jsxs("div", { className: "workspace-alert-bar", children: [_jsx("span", { children: updateStatusNote || "A newer desktop version is available on GitHub Releases." }), _jsx("button", { className: "primary-button", type: "button", onClick: () => void handleOpenManualUpdate(), children: "Download latest installer" })] })) : null, _jsxs("main", { className: `notes-shell${activeWorkspace === "calendar" && isCalendarWorkspaceFullScreen ? " notes-shell-calendar-fullscreen" : ""}${activeWorkspace === "notes" && isNotesSessionsOpen ? " notes-shell-with-sessions" : ""}${activeWorkspace === "notes" ? " notes-shell-notes-mode" : ""}${activeWorkspace === "notebook" ? " notes-shell-notebook" : ""}${SINGLE_PANE_WORKSPACES.includes(activeWorkspace) ? " notes-shell-single-pane" : ""}`, children: [_jsxs("section", { className: `workspace-canvas${activeWorkspace === "calendar" ? " workspace-canvas-calendar" : ""}${activeWorkspace === "notebook" ? " workspace-canvas-notebook" : ""}`, children: [activeWorkspace !== "notes" && !(activeWorkspace === "notebook" || activeWorkspace === "calendar" || activeWorkspace === "now") ? (_jsx("div", { className: "workspace-header card", children: _jsx("div", { className: "card-header", children: _jsxs("div", { children: [_jsx("div", { className: "topbar-eyebrow", children: "Workspace" }), _jsx("h2", { children: WORKSPACE_ITEMS.find((item) => item.id === activeWorkspace)?.label || "Workspace" })] }) }) })) : null, activeWorkspace === "notebook" ? (_jsx(NotebookWorkspace, { sessions: activeSessions, todos: snapshot.todos, runningTodoIds: getRunningTodoIds(snapshot.timelogs), activeSession: activeSession, structureOptions: structureOptions, isTimeTracking: isActiveSessionTimeTracking, isRecordingAudio: isRecordingAudio, isTranscribingAudio: isTranscribingAudio, isGenerating: isGenerating, recordingStatusNote: recordingStatusNote, onSelect: (sessionId) => {
                                                setSelectedOutputVersionId(null);
                                                setActiveSessionId(sessionId);
                                            }, onCreate: () => void handleCreateNotebookPage(), onDelete: (sessionId) => void deleteSession(sessionId), onAddTodo: (description, options) => void addTodo(description, options), onSaveTodo: (todo) => void saveTodo(todo), onDeleteTodo: (todoId) => void deleteTodo(todoId), onAddNoteForTodo: (todoId) => {
                                            ...
                                            }, onToggleTodoTime: (todoId, isRunning) => void (isRunning ? stopTimeTracking("todo", todoId) : startTimeTracking("todo", todoId)), onChange: handleCaptureSessionChange, onToggleRecording: () => void (isRecordingAudio ? handleStopRecording() : handleStartRecording("microphone")), onUploadAudio: () => void handleImportAudio(), onTranscribeAudio: () => void handleTranscribeAudio(), onGenerateOutput: () => void handleGenerate(), onOpenInNotes: (view) => openNotesTarget({
                                                sessionId: activeSession.id,
                                                view,
                                                returnWorkspace: "notebook",
                                                status: "Opened the Notebook page in the full Notes workspace.",
                                            }), onToggleTimeTracking: () => void toggleActiveSessionTimeTracking(), outputContent: (_jsx(OutputWorkspace, { session: activeSession, template: activeTemplate, displayedOutput: displayedOutput, layoutPresetId: snapshot.settings.outputLayoutPresetId, outputVersions: activeOutputVersions, selectedOutputVersionId: selectedOutputVersionId, attachments: activeAttachments, presentation: "minimal", showPresentationActions: false, showPanelHeading: false, showDetailsSection: false, onChange: (session) => void handleOutputWorkspaceChange(session), savedPeople: snapshot.settings.savedParticipants, suggestedPeople: suggestedPeople, savedProjects: snapshot.settings.savedProjects, suggestedProjects: suggestedProjects, savedDomains: snapshot.settings.savedDomains, suggestedDomains: suggestedDomains, savedActivities: snapshot.settings.savedActivities, suggestedActivities: suggestedActivities, structureOptions: structureOptions, savedTags: snapshot.settings.savedTags, suggestedTags: suggestedTags, isPrimaryActionRunning: outputActionConfig.isPrimaryRunning, isSecondaryActionRunning: outputActionConfig.isSecondaryRunning, isRevising: isRevising, onPrimaryAction: outputActionConfig.onPrimary, onSecondaryAction: outputActionConfig.onSecondary, onCopyOutput: () => void handleCopyOutput(), onTranslate: () => void handleTranslate(), onRevise: (instructions) => void handleRevise(instructions), onRevertOutputVersion: handleRevertOutputVersion, onOpenOutputVersion: handleOpenOutputVersion, onOpenLatestOutputVersion: handleOpenLatestOutputVersion, onExportText: () => exportOutputAsText({ title: activeSession.title, output: displayedOutput }), onExportMarkdown: () => exportOutputAsMarkdown({ title: activeSession.title, output: displayedOutput }), onExportHtml: () => exportOutputAsHtml({ title: activeSession.title, output: displayedOutput, attachments: activeAttachments, layoutPresetId: snapshot.settings.outputLayoutPresetId }), onExportDocx: () => void exportOutputAsDocx({ title: activeSession.title, output: displayedOutput, attachments: activeAttachments, layoutPresetId: snapshot.settings.outputLayoutPresetId }), onExportPdf: () => void exportOutputAsPdf({ title: activeSession.title, output: displayedOutput, attachments: activeAttachments, layoutPresetId: snapshot.settings.outputLayoutPresetId }), ruleSuggestions: visibleRuleSuggestions.filter((entry) => !dismissedRuleSuggestionIds.includes(entry.id)), onAcceptRuleSuggestion: (suggestionId) => void handleAcceptVisibleRuleSuggestion(suggestionId), onDismissRuleSuggestion: handleDismissVisibleRuleSuggestion, onIgnoreRuleSuggestion: (suggestionId) => void handleIgnoreVisibleRuleSuggestion(suggestionId), primaryActionLabel: outputActionConfig.primaryLabel, secondaryActionLabel: outputActionConfig.secondaryLabel, emptyStatePrimaryLabel: outputActionConfig.emptyStatePrimaryLabel, emptyStateSecondaryLabel: outputActionConfig.emptyStateSecondaryLabel })) })) : activeWorkspace === "todos" ? (_jsx(TodosWorkspace, { todos: snapshot.todos, checklists: snapshot.checklists, checklistTemplates: snapshot.checklistTemplates, checklistRecurrences: snapshot.checklistRecurrences, activities: snapshot.activities, timeLogs: snapshot.timelogs, settings: snapshot.settings, structureOptions: structureOptions, savedPeople: snapshot.settings.savedParticipants, suggestedPeople: suggestedPeople, requestedTodoId: requestedTodoId, requestedDomain: requestedTodoDomain, requestedProject: requestedTodoProject, onEditorClose: returnFromLinkedDetail, onToggle: (todo) => void saveTodo(todo), onAdd: (description, options) => void addTodo(description, options), onSave: (todo) => void saveTodo(todo), onDelete: (id) => void deleteTodo(id), onCreateChecklist: (todoId, title) => void createChecklist("todo", todoId, title), onCreateChecklistFromTemplate: (todoId, templateId) => void createChecklistFromTemplate("todo", todoId, templateId), onCreateChecklistRecurrence: (todoId, templateId, cadence) => void createChecklistRecurrence("todo", todoId, templateId, cadence), onDeleteChecklistRecurrence: (id) => void deleteChecklistRecurrence(id), onSaveChecklist: (checklist) => void saveChecklist(checklist), onDeleteChecklist: (id) => void deleteChecklist(id), onCreateChecklistTemplate: (title, category, items) => void createChecklistTemplate(title, category, items), onSaveChecklistTemplate: (template) => void saveChecklistTemplate(template), onDeleteChecklistTemplate: (id) => void deleteChecklistTemplate(id), onConvertToActivity: (todo) => void convertTodoToActivity(todo), onSaveTimeLog: (timeLog) => void saveTimeLog(timeLog), onDeleteTimeLog: (id) => void deleteTimeLog(id), onStartTracking: (targetType, targetId) => void startTimeTracking(targetType, targetId), onStopTracking: (targetType, targetId) => void stopTimeTracking(targetType, targetId), onSaveSettings: (settings) => void saveSettings(settings), onOpenActivityDetail: (activityId) => openActivityFromLink(activityId, "todos") })) : activeWorkspace === "now" ? (_jsx(NowWorkspace, { todos: snapshot.todos, activities: snapshot.activities, timeLogs: snapshot.timelogs, calendarItems: snapshot.calendarItems ?? [], settings: snapshot.settings, onToggleTodo: (todo) => void saveTodo(todo), onStartTracking: (targetType, targetId) => void startTimeTracking(targetType, targetId), onStopTracking: (targetType, targetId) => void stopTimeTracking(targetType, targetId), onOpenTodoDetail: (todoId) => openTodoDetailFromLink(todoId, "now"), onOpenActivityDetail: (activityId) => openActivityFromLink(activityId, "now"), onOpenProject: (project) => openLinkedDestination({
                                                workspace: "time",
                                                timeProject: project,
                                                returnWorkspace: "now",
                                                status: `Opened ${project} in Time.`,
                                            }), onSaveSettings: (nextSettings) => void saveSettings(nextSettings) })) : activeWorkspace === "calendar" ? (_jsx(CalendarWorkspace, { todos: snapshot.todos, checklists: snapshot.checklists, activities: snapshot.activities, timeLogs: snapshot.timelogs, calendarItems: snapshot.calendarItems ?? [], settings: snapshot.settings, openRevision: calendarOpenRevision, structureOptions: structureOptions, savedPeople: snapshot.settings.savedParticipants, linkedSessionStateByActivity: linkedSessionStateByActivity, linkedSessionStateByTodo: linkedSessionStateByTodo, onSaveSettings: (settings) => void saveSettings(settings), onCreateFromText: (date, startSlot, value, options) => createCalendarEntryFromText(date, startSlot, value, options), onMoveItem: (id, date, startSlot) => void moveCalendarItem(id, date, startSlot), onSaveTodo: (todo) => void saveTodo(todo), onDeleteTodo: (id) => void deleteTodo(id), onCreateChecklist: (todoId, title) => void createChecklist("todo", todoId, title), onSaveChecklist: (checklist) => void saveChecklist(checklist), onDeleteChecklist: (id) => void deleteChecklist(id), onSaveActivity: (activity) => void saveActivity(activity), onDeleteActivity: (id) => void deleteActivity(id), onConvertTodoToMeeting: (todo, options) => void convertTodoToActivity(todo, {
                                                type: "meeting",
                                                date: options.date,
                                                startTime: options.startTime,
                                                endTime: options.endTime,
                                            }), onUpdateCalendarItem: (id, updates) => void updateCalendarItem(id, updates), onSaveTimeLog: (timeLog) => void saveTimeLog(timeLog), onStartTracking: (targetType, targetId) => void startTimeTracking(targetType, targetId), onStopTracking: (targetType, targetId) => void stopTimeTracking(targetType, targetId), onOpenTodoWorkspace: () => setActiveWorkspace("todos"), onOpenTodoDetail: (todoId) => openTodoDetailFromLink(todoId, "calendar"), onOpenActivityWorkspace: (activityId) => openActivityFromLink(activityId, "calendar"), onOpenActivityDetail: (activityId) => openActivityFromLink(activityId, "calendar"), onOpenSession: (sessionId, openedCalendarItemId) => {
                                            ...
                                            }) })) : activeWorkspace === "analytics" ? (_jsx(AnalyticsWorkspace, { todos: snapshot.todos, archivedTasks: snapshot.archivedTasks, activities: snapshot.activities, timeLogs: snapshot.timelogs, settings: snapshot.settings, onOpenTodoDetail: (todoId) => openTodoDetailFromLink(todoId, "analytics"), onOpenActivityDetail: (activityId) => openActivityFromLink(activityId, "analytics"), onSaveTimeLog: (timeLog) => void saveTimeLog(timeLog), onSaveTodo: (todo) => void saveTodo(todo), onSaveActivity: (activity) => void saveActivity(activity) })) : activeWorkspace === "structure" ? (_jsx(StructureWorkspace, { activities: snapshot.activities, todos: snapshot.todos, checklists: snapshot.checklists, checklistTemplates: snapshot.checklistTemplates, checklistRecurrences: snapshot.checklistRecurrences, timeLogs: snapshot.timelogs, savedDomains: snapshot.settings.savedDomains, savedProjects: snapshot.settings.savedProjects, projectLinks: snapshot.settings.projectLinks, onAddDomain: (domain) => void saveSettings({
                                                ...snapshot.settings,
                                                ...
                                                project,
                                                domain,
                                            }), onCreateProjectChecklist: (project, title) => void createChecklist("project", project, title), onCreateProjectChecklistFromTemplate: (project, templateId) => void createChecklistFromTemplate("project", project, templateId), onCreateProjectChecklistRecurrence: (project, templateId, cadence) => void createChecklistRecurrence("project", project, templateId, cadence), onDeleteChecklistRecurrence: (id) => void deleteChecklistRecurrence(id), onSaveChecklist: (checklist) => void saveChecklist(checklist), onDeleteChecklist: (id) => void deleteChecklist(id), onCreateChecklistTemplate: (title, category, items) => void createChecklistTemplate(title, category, items), onSaveChecklistTemplate: (template) => void saveChecklistTemplate(template), onDeleteChecklistTemplate: (id) => void deleteChecklistTemplate(id), onRenameProject: (previousValue, nextValue) => void renameProjectValue(previousValue, nextValue), onAssignProjectDomain: (project, domain) => void saveSettings({
                                                ...snapshot.settings,
                                                projectLinks: domain.trim()
                                                    ? [
                                                        ...snapshot.settings.projectLinks.filter((entry) => entry.project !== project.trim()),
                                            ...
                                            }), onOpenActivityDetail: (activityId) => openActivityFromLink(activityId, "structure"), onOpenTodoDetail: (todoId) => openTodoDetailFromLink(todoId, "structure") })) : activeWorkspace === "assistant" ? (_jsx(AssistantWorkspace, { snapshot: snapshot, onOpenSettings: () => openSettingsSection("ai"), onSaveSettings: (settings) => saveSettings(settings) })) : activeWorkspace !== "notes" ? (_jsxs("div", { className: "card empty-state-card", children: [_jsx("h2", { children: "Coming next" }), _jsx("p", { children: WORKSPACE_ITEMS.find((item) => item.id === activeWorkspace)?.description || "This workspace is planned for a later phase." }), _jsxs("ol", { className: "empty-state-steps", children: [_jsx("li", { children: "Return to Notes from the left rail whenever you want to work now." }), _jsx("li", { children: "Use Ctrl/Cmd+K to reach settings, sessions, and future actions quickly." }), _jsx("li", { children: "This workspace will use the same center-canvas plus right-inspector pattern when it ships." })] })] })) : (_jsxs("div", { className: "notes-pwa-workbench", children: [linkedDetailReturnWorkspace ? (_jsxs("div", { className: "notes-pwa-toolbar", children: [_jsxs("button", { className: "shell-button", type: "button", onClick: returnFromLinkedDetail, children: ["Back to ", linkedDetailReturnWorkspace === "notebook" ? "Notebook" : linkedDetailReturnWorkspace === "calendar" ? "Calendar" : linkedDetailReturnWorkspace === "time" ? "Time" : linkedDetailReturnWorkspace === "now" ? "Now" : linkedDetailReturnWorkspace === "structure" ? "Structure" : "previous workspace"] }), activeLinkedActivity ? (_jsxs("button", { className: "shell-button", type: "button", onClick: () => openActivityFromLink(activeLinkedActivity.id, "notes"), children: ["Linked activity: ", activeLinkedActivity.description] })) : null] })) : activeLinkedActivity ? (_jsx("div", { className: "notes-pwa-toolbar", children: _jsxs("button", { className: "shell-button", type: "button", onClick: () => openActivityFromLink(activeLinkedActivity.id, "notes"), children: ["Linked activity: ", activeLinkedActivity.description] }) })) : null, _jsx("div", { className: "calendar-session-overlay-tabs notes-workspace-tabs", role: "tablist", "aria-label": "Notes workspace tabs", children: ["capture", "output", "details"].map((tab) => (_jsx("button", { type: "button", className: "calendar-session-overlay-tab", "data-active": notesWorkspaceTab === tab, "aria-selected": notesWorkspaceTab === tab, onClick: () => {
                                                            setNotesWorkspaceTab(tab);
                                                            if (tab === "capture" || tab === "output") {
                                                                setActiveView(tab);
                                                            }
                                                        }, children: tab === "capture" ? "Capture" : tab === "output" ? "Output" : "Details" }, tab))) }), _jsxs("div", { ref: notesLayoutRef, className: "notes-pwa-tab-panel", children: [notesWorkspaceTab === "capture" ? (_jsx("div", { className: "notes-pwa-capture", children: _jsx(SessionEditor, { session: activeSession, templates: snapshot.templates, allowedTemplateCaptureModes: linkedDetailReturnWorkspace === "notebook" ? ["quick-note", "meeting-note"] : undefined, attachments: activeAttachments, presentation: "minimal", showPresentationActions: false, savedPeople: snapshot.settings.savedParticipants, suggestedPeople: suggestedPeople, savedProjects: snapshot.settings.savedProjects, suggestedProjects: suggestedProjects, savedDomains: snapshot.settings.savedDomains, suggestedDomains: suggestedDomains, savedActivities: snapshot.settings.savedActivities, suggestedActivities: suggestedActivities, structureOptions: structureOptions, savedTags: snapshot.settings.savedTags, suggestedTags: suggestedTags, isTranscribingAudio: isTranscribingAudio, recordingMode: recordingMode, isRecordingAudio: isRecordingAudio, recordingStatusNote: recordingStatusNote, generationLog: generationLog, onClearGenerationLog: () => setGenerationLog([]), onChange: handleCaptureSessionChange, onImportImage: () => void handleImportImage(), onCreateInlineImageAttachment: (file) => handleCreateInlineImageAttachment(file), onImportAudio: () => void handleImportAudio(), onTranscribeAudio: () => void handleTranscribeAudio(), onChangeRecordingMode: setRecordingMode, onStartRecording: (mode) => void handleStartRecording(mode), onStopRecording: () => void handleStopRecording(), onImportTranscript: () => void handleImportTranscript(), onRemoveAttachment: (attachmentId) => void handleRemoveAttachment(attachmentId), onUpdateAttachment: (attachment) => void handleUpdateAttachment(attachment), onOpenDetails: () => setNotesWorkspaceTab("details"), onCreateSessionFromTemplate: (templateId) => void handleCreateSessionFromTemplate(templateId), onOpenInstructions: () => openOverlay("instructions") }) })) : null, notesWorkspaceTab === "output" ? (_jsx("div", { className: "notes-pwa-output", children: _jsx(OutputWorkspace, { session: activeSession, template: activeTemplate, displayedOutput: displayedOutput, layoutPresetId: snapshot.settings.outputLayoutPresetId, outputVersions: activeOutputVersions, selectedOutputVersionId: selectedOutputVersionId, attachments: activeAttachments, presentation: "minimal", showPresentationActions: false, onChange: (session) => void handleOutputWorkspaceChange(session), savedPeople: snapshot.settings.savedParticipants, suggestedPeople: suggestedPeople, savedProjects: snapshot.settings.savedProjects, suggestedProjects: suggestedProjects, savedDomains: snapshot.settings.savedDomains, suggestedDomains: suggestedDomains, savedActivities: snapshot.settings.savedActivities, suggestedActivities: suggestedActivities, structureOptions: structureOptions, savedTags: snapshot.settings.savedTags, suggestedTags: suggestedTags, isPrimaryActionRunning: outputActionConfig.isPrimaryRunning, isSecondaryActionRunning: outputActionConfig.isSecondaryRunning, isRevising: isRevising, onPrimaryAction: outputActionConfig.onPrimary, onSecondaryAction: outputActionConfig.onSecondary, onCopyOutput: () => void handleCopyOutput(), onTranslate: () => void handleTranslate(), onRevise: (instructions) => void handleRevise(instructions), onRevertOutputVersion: handleRevertOutputVersion, onOpenOutputVersion: handleOpenOutputVersion, onOpenLatestOutputVersion: handleOpenLatestOutputVersion, onExportText: () => exportOutputAsText({ title: activeSession.title, output: displayedOutput }), onExportMarkdown: () => exportOutputAsMarkdown({ title: activeSession.title, output: displayedOutput }), onExportHtml: () => exportOutputAsHtml({ title: activeSession.title, output: displayedOutput, attachments: activeAttachments, layoutPresetId: snapshot.settings.outputLayoutPresetId }), onExportDocx: () => void exportOutputAsDocx({ title: activeSession.title, output: displayedOutput, attachments: activeAttachments, layoutPresetId: snapshot.settings.outputLayoutPresetId }), onExportPdf: () => void exportOutputAsPdf({ title: activeSession.title, output: displayedOutput, attachments: activeAttachments, layoutPresetId: snapshot.settings.outputLayoutPresetId }), ruleSuggestions: visibleRuleSuggestions.filter((entry) => !dismissedRuleSuggestionIds.includes(entry.id)), onAcceptRuleSuggestion: (suggestionId) => void handleAcceptVisibleRuleSuggestion(suggestionId), onDismissRuleSuggestion: handleDismissVisibleRuleSuggestion, onIgnoreRuleSuggestion: (suggestionId) => void handleIgnoreVisibleRuleSuggestion(suggestionId), primaryActionLabel: outputActionConfig.primaryLabel, secondaryActionLabel: outputActionConfig.secondaryLabel, emptyStatePrimaryLabel: outputActionConfig.emptyStatePrimaryLabel, emptyStateSecondaryLabel: outputActionConfig.emptyStateSecondaryLabel, linkedActivity: activeLinkedActivity, onOpenLinkedActivity: (activityId) => openActivityFromLink(activityId, "notes"), onAddFollowUpTodo: (description, options) => void addTodo(description, {
                                                                    ...getMeetingTodoDefaults(),
                                                                    ...options,
                                                                }), onAddFollowUpMeeting: (description, options) => void addActivity(description, "meeting", options) }) })) : null, notesWorkspaceTab === "details" ? (_jsx("div", { className: "card calendar-session-details-card notes-workspace-details-card", children: _jsxs("div", { className: "calendar-session-details-grid", children: [_jsxs("div", { className: "field field-wide", children: [_jsx("label", { htmlFor: "notes-workspace-session-title", children: "Title" }), _jsx("input", { id: "notes-workspace-session-title", value: activeSession.title, onChange: (event) => handleCaptureSessionChange({ ...activeSession, title: event.target.value }), placeholder: "Session title" })] }), _jsxs("div", { className: "field", children: [_jsx("label", { htmlFor: "notes-workspace-template", children: "Template" }), _jsx("select", { id: "notes-workspace-template", value: activeTemplate?.id ?? "", onChange: (event) => handleCalendarOverlayTemplateChange(event.target.value), children: getTemplatesForCaptureMode(snapshot.templates, activeSession.captureMode).map((template) => (_jsx("option", { value: template.id, children: template.name }, template.id))) })] }), _jsxs("div", { className: "field", children: [_jsx("label", { htmlFor: "notes-workspace-date", children: "Date" }), _jsx(DateInput, { id: "notes-workspace-date", value: activeSession.date, onChange: (event) => handleCaptureSessionChange({ ...activeSession, date: event.target.value }) })] }), _jsxs("div", { className: "field", children: [_jsx("label", { htmlFor: "notes-workspace-start", children: "Start" }), _jsx(DeferredTimeInput, { id: "notes-workspace-start", value: activeSession.startTime, onCommit: (value) => handleCaptureSessionChange({ ...activeSession, startTime: value }) })] }), _jsxs("div", { className: "field", children: [_jsx("label", { htmlFor: "notes-workspace-end", children: "End" }), _jsx(DeferredTimeInput, { id: "notes-workspace-end", value: activeSession.endTime, onCommit: (value) => handleCaptureSessionChange({ ...activeSession, endTime: value }) })] }), _jsxs("div", { className: "field field-wide", children: [_jsx("label", { htmlFor: "notes-workspace-people", children: "People" }), _jsx(PeoplePicker, { value: activeSession.participantText, savedPeople: snapshot.settings.savedParticipants, suggestedPeople: suggestedPeople, onChange: (value) => handleCaptureSessionChange({ ...activeSession, participantText: value }), placeholder: "Search or add people" })] }), _jsx("div", { className: "field field-wide metadata-triplet", children: _jsxs("div", { className: "metadata-triplet-grid", children: [_jsxs("div", { className: "field metadata-subfield", children: [_jsx("label", { htmlFor: "notes-workspace-domain", children: "Domain" }), _jsx(TokenPicker, { value: activeSession.domain, savedOptions: structureOptions.domains.length ? structureOptions.domains : snapshot.settings.savedDomains, suggestedOptions: suggestedDomains, placeholder: "Search or add domain", suggestionSummary: "Recent domains", suggestionBadgeText: "From saved Domains", mode: "single", onChange: handleCalendarOverlayDomainChange })] }), _jsxs("div", { className: "field metadata-subfield", children: [_jsx("label", { htmlFor: "notes-workspace-project", children: "Project" }), _jsx(TokenPicker, { value: activeSession.project, savedOptions: (() => {
                                                                                            ...
                                                                                            })(), suggestedOptions: suggestedProjects.filter((project) => new Set((() => {
                                                                                                const options = getProjectsForDomain(structureOptions, activeSession.domain);
                                                                                                return options.length ? options : snapshot.settings.savedProjects;
                                                                                            })()).has(project)), placeholder: "Search or add project", suggestionSummary: "Recent projects", suggestionBadgeText: "From saved Projects", mode: "single", onChange: handleCalendarOverlayProjectChange })] }), _jsxs("div", { className: "field metadata-subfield", children: [_jsx("label", { htmlFor: "notes-workspace-activity", children: "Activity" }), _jsx(TokenPicker, { value: activeSession.activity, savedOptions: (() => {
