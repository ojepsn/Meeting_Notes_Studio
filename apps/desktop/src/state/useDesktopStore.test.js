...
const buildCalendarTodo = (id, targetId, date, startSlot) => ({
    id,
    targetType: "todo",
    targetId,
    date,
...
const buildCalendarActivity = (id, date, startSlot, durationSlots) => ({
    id,
    targetType: "activity",
...
describe("rollForwardOverdueCalendarTodos", () => {
    ...
    it("places two overdue todos per early row when there are more todos than free morning rows", () => {
        const snapshot = createDefaultSnapshot();
        snapshot.todos = Array.from({ length: 25 }, (_, index) => buildTodo(`todo-${index}`, `Todo ${index}`, "2026-04-21"));
        snapshot.calendarItems = snapshot.todos.map((todo, index) => buildCalendarTodo(`calendar-${index}`, todo.id, "2026-04-21", 60 + index));
        const result = rollForwardOverdueCalendarTodos(snapshot, "2026-04-22");
...
describe("isSuspiciouslyReducedSnapshot", () => {
    it("flags snapshots where many calendar todo rows have lost their source todos", () => {
        ...
        snapshot.calendarItems = Array.from({ length: 12 }, (_, index) => buildCalendarTodo(`calendar-${index + 1}`, `todo-${index + 1}`, "2026-05-19", 96 + index));
    ...
    it("does not flag snapshots when calendar todo rows still have matching todo records", () => {
        const snapshot = createDefaultSnapshot();
        snapshot.todos = Array.from({ length: 12 }, (_, index) => buildTodo(`todo-${index + 1}`, `Todo ${index + 1}`, "2026-05-19"));
        snapshot.calendarItems = snapshot.todos.map((todo, index) => buildCalendarTodo(`calendar-${index + 1}`, todo.id, "2026-05-19", 96 + index));
