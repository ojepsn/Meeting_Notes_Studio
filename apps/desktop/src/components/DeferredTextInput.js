...
const useDeferredTextDraft = (value, onCommit, commitDelayMs) => {
    ...
    return {
        ...
        setIsFocused,
        updateDraft: (nextValue, shouldSchedule = true) => {
...
export const DeferredTextInput = ({ value, onCommit, onDraftChange, commitDelayMs = 400, onBlur, onFocus, onKeyDown, onCompositionStart, onCompositionEnd, ...props }) => {
    ...
    return (_jsx("input", { ...props, value: state.draft, onChange: (event) => {
            ...
            onBlur?.(event);
        }, onKeyDown: handleKeyDown, onCompositionStart: (event) => {
...
export const DeferredTextArea = ({ value, onCommit, onDraftChange, commitDelayMs = 400, onBlur, onFocus, onKeyDown, onCompositionStart, onCompositionEnd, ...props }) => {
    const state = useDeferredTextDraft(value, onCommit, commitDelayMs);
    const isComposingRef = useRef(false);
    return (_jsx("textarea", { ...props, value: state.draft, onChange: (event) => {
            const nextValue = event.target.value;
            state.updateDraft(nextValue, !isComposingRef.current);
            onDraftChange?.(nextValue);
        }, onFocus: (event) => {
