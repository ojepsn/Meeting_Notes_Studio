...
type LegacyTextModelId = "gpt-5-mini" | "gpt-5" | "gpt-4.1";
...
const DEFAULT_TEXT_MODEL_ID: TextModelId = "gpt-5.4-mini";
const DEFAULT_TRANSCRIPTION_MODEL_ID: TranscriptionModelId = "gpt-4o-mini-transcribe";

const CURRENT_TEXT_MODEL_IDS = new Set<TextModelId>(["gpt-5.4", "gpt-5.4-mini", "gpt-5.4-nano", "gpt-5.4-pro"]);
...
const parseTranscriptionModelRecommendation = (
  speechDocsText: string,
  fallback: TranscriptionModelPricingEntry,
): TranscriptionModelPricingEntry => {
  const diarizeSentence = findFirstSentence(speechDocsText, [/gpt-4o-transcribe-diarize produces speaker-aware transcripts\./i]);
  const sharedSupportSentence = findFirstSentence(
    speechDocsText,
    [/gpt-4o-transcribe and gpt-4o-mini-transcribe support json or text responses and allow prompts and logprobs\./i],
  );
  ...
  if (fallback.id === "gpt-4o-transcribe-diarize") {
    return {
      ...
      recommendation:
        "Use this when speaker attribution is more important than prompt support or the absolute lowest cost.",
    };
  }
...
export const parseModelPricingPage = ({
  ...
  const transcriptionModels = DEFAULT_TRANSCRIPTION_MODEL_PRICING.map((fallback) => {
    const withPricing = parseTranscriptionModelPricing(
      normalizedPricingText,
      currentTranscriptionModels.get(fallback.id) || fallback,
      pricingDate,
    );
...
export const fetchLatestModelPricingSnapshot = async ({
  ...
  const [pricingResponse, modelsResponse, latestModelResponse, speechResponse] = await Promise.all(
    OPENAI_DOC_URLS.map((url) => fetch(url, { cache: "no-store" })),
  );
  ...
  const parseHtmlText = (html: string) => new DOMParser().parseFromString(html, "text/html").body.textContent || "";
