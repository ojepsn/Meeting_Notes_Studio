...
export const fetchLatestModelPricingSnapshot = async ({
  ...
  const [compareResponse, modelsResponse, latestModelResponse, gptTranscribeResponse, gpt4oTranscribeResponse, gpt4oMiniTranscribeResponse] =
    await Promise.all(OPENAI_DOC_URLS.map((url) => fetch(url, { cache: "no-store" })));
  ...
  const [compareHtml, modelsHtml, latestModelHtml, gptTranscribeHtml, gpt4oTranscribeHtml, gpt4oMiniTranscribeHtml] = await Promise.all(
