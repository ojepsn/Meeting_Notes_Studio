...
const getLastSundayOfMonthUtc = (year: number, monthIndex: number) => {
...
export const getStockholmOffsetMinutes = (value = new Date()) => {
  ...
  const dstStart = Date.UTC(
    year,
    2,
    dstStartDate.getUTCDate(),
    STOCKHOLM_DST_START_HOUR_UTC,
    0,
    0,
    0,
  );
  const dstEnd = Date.UTC(
    year,
    9,
    dstEndDate.getUTCDate(),
    STOCKHOLM_DST_END_HOUR_UTC,
    0,
    0,
    0,
  );
...
const toStockholmClock = (value = new Date()) =>
  new Date(value.getTime() + getStockholmOffsetMinutes(value) * 60 * 1000);
...
export const formatStockholmDateParts = (year: number, month: number, day: number) =>
...
export const parseIsoDateUtc = (date: string) => {
...
export const formatIsoDateUtc = (value: Date) =>
...
export const daysBetweenIso = (fromDate: string, toDate: string) =>
  Math.round((parseIsoDateUtc(toDate).getTime() - parseIsoDateUtc(fromDate).getTime()) / DAY_MS) || 0;

export const formatStockholmDayLabel = (date: string) => {
