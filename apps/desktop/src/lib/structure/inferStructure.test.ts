...
describe("inferStructure", () => {
  it("uses a learned exact-title rule before fuzzy historical matching", () => {
    const rules = upsertStructureInferenceRule(
      [],
      "Monthly time reporting staff",
      "todo",
      {
        domain: "Operations",
        project: "HR",
  ...
  it("updates an existing learned rule when the user corrects the structure", () => {
    const initialRules = upsertStructureInferenceRule(
      [],
      "CS meeting",
      "meeting",
      {
    ...
    const correctedRules = upsertStructureInferenceRule(
      initialRules,
      "CS meeting",
      "meeting",
      {
