...
import { createAgentPlatformSession, parseSseBlocks, postAgentPlatformMessage, runAgentPlatformTurn, } from "./agentPlatformHttp";
describe("agent platform HTTP client", () => {
    it("creates sessions and posts messages against the documented API", async () => {
        ...
        expect(fetchMock).toHaveBeenNthCalledWith(1, "http://127.0.0.1:8088/api/sessions", expect.objectContaining({ method: "POST" }));
        expect(fetchMock).toHaveBeenNthCalledWith(2, "http://127.0.0.1:8088/api/sessions/session-1/messages", expect.objectContaining({ method: "POST" }));
    });
    it("parses server-sent events from agent_platform", () => {
        expect(parseSseBlocks('event: text_delta\ndata: {"type":"text_delta","text":"Hello"}\n\n')).toEqual([{ event: "text_delta", data: { type: "text_delta", text: "Hello" } }]);
    });
    it("streams text deltas and returns final text when present", async () => {
        ...
        await expect(runAgentPlatformTurn({
