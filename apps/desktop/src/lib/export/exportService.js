...
export const buildHtmlMarkup = (entries) => {
    const parts = [];
    let activeList = null;
    const closeList = () => {
        if (activeList) {
...
const getNarrativeColumnOffset = (layout, contentWidth) => layout.variant === "narrative-memo" ? Math.max(0, (contentWidth - 390) / 2) : 0;
const getNarrativeColumnWidth = (layout, contentWidth) => layout.variant === "narrative-memo" ? Math.min(contentWidth, 390) : contentWidth;
        ...
        sections: [
            {
                children: [
                    new Paragraph({
                        ...
                        border: layout.variant === "executive-brief"
                            ? {
                                top: {
                                    color: layout.style.headingColor.replace("#", ""),
                                    style: BorderStyle.SINGLE,
...
export const exportOutputAsPdf = async ({ title, output, attachments = [], layoutPresetId }) => {
    ...
    structuredOutput.forEach((entry) => {
        ...
        const lines = pdf.splitTextToSize(`${prefix}${text}`, contentWidth - indent - (layout.variant === "modern-minutes" && isHeading ? 18 : 0));
