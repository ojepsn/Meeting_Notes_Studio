    ...
    ruleSuggestions: Array.isArray(settings.ruleSuggestions)
        ? settings.ruleSuggestions
            ...
            observedSessionIds: Array.isArray(entry?.observedSessionIds)
                ? entry.observedSessionIds.filter((value) => typeof value === "string" && value.trim().length > 0)
                : [],
...
class TauriSqliteRepository {
    ...
    async loadSessions() {
        ...
        return rows.map((row) => normalizeSessionRecord({
            ...
            outputVersions: row.output_versions
                ? JSON.parse(row.output_versions)
                : [],
    ...
    async loadTimeLogs() {
        const db = await this.getDb();
        const rows = await db.select("SELECT id, payload_json, target_type, target_id, log_date, start_time, end_time, duration_minutes, created_at, updated_at FROM timelogs ORDER BY updated_at DESC");
        return rows.map((row) => {
            const payload = parsePayload(row.payload_json);
    ...
    async loadCalendarItems() {
        const db = await this.getDb();
        const rows = await db.select("SELECT id, payload_json, target_type, target_id, schedule_date, start_slot, duration_slots, created_at, updated_at FROM calendar_items ORDER BY updated_at DESC");
        return rows.map((row) => {
            const payload = parsePayload(row.payload_json);
