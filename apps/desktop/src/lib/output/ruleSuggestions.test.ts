...
import {
  ...
  restoreIgnoredRuleSuggestion,
} from "./ruleSuggestions";

const createSettings = (): LocalAppSettings => ({
...
describe("rule suggestion helpers", () => {
  it("collects safe abbreviation suggestions from repeated shorthand", () => {
    const observations = collectRuleSuggestionObservations(
      createSession(),
      createSettings(),
      "Follow-up mtg booked for next week.",
    );
  ...
  it("collects preferred participant-name suggestions only when the mapping is unambiguous", () => {
    const observations = collectRuleSuggestionObservations(
      createSession({ participantText: "Ola Jeppsson, Anna Smith" }),
      createSettings(),
      "Ola will send the revised draft.",
    );
  ...
  it("does not suggest ambiguous first-name mappings", () => {
    const observations = collectRuleSuggestionObservations(
      createSession({ participantText: "Ann Smith, Ann Jones" }),
      createSettings(),
      "Ann will follow up.",
    );
  ...
  it("accepting a suggestion stores the reusable rule", () => {
    ...
    expect(accepted.preferredParticipantNames).toContainEqual(
      expect.objectContaining({ shortForm: "Ola", fullName: "Ola Jeppsson" }),
    );
