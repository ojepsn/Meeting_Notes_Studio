...
const SAFE_ABBREVIATION_SUGGESTIONS: Record<string, string> = {
  mtg: "meeting",
  mins: "minutes",
  wk: "week",
  wks: "weeks",
...
export const collectRuleSuggestionObservations = (
...
export const mergeRuleSuggestionObservations = (
  settings: LocalAppSettings,
  sessionId: string,
...
export const acceptRuleSuggestion = (settings: LocalAppSettings, suggestionId: string): LocalAppSettings => {
  ...
  if (suggestion.type === "preferred_name") {
    const exists = nextSettings.preferredParticipantNames.some(
      (entry) =>
...
export const ignoreRuleSuggestion = (
  settings: LocalAppSettings,
  suggestionId: string,
  { forever = true }: { forever?: boolean } = {},
): LocalAppSettings => ({
  ...settings,
  ruleSuggestions: settings.ruleSuggestions.map((entry) =>
    entry.id === suggestionId
...
export const restoreIgnoredRuleSuggestion = (settings: LocalAppSettings, suggestionId: string): LocalAppSettings => ({
  ...settings,
  ruleSuggestions: settings.ruleSuggestions.map((entry) =>
    entry.id === suggestionId
      ? {
